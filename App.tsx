...
type Page = 'lineup' | 'players' | 'organiser';
...
const App: React.FC = () => {
  ...
  // URL routing for the team-approval page (deep link from the bot DM)
  const approvalToken = (() => {
  ...
  // URL routing for the weekly confirmation page (deep link from the confirmation DM)
  const confirmToken = (() => {
    const m = window.location.pathname.match(/^\/confirm\/([^/?#]+)/);
    return m ? decodeURIComponent(m[1]) : null;
  })();
  if (confirmToken) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-slate-500 text-[10px] font-black tracking-[0.2em] uppercase mt-1">
                Weekly signups, reminders & teams
              </p>
            </div>
          </div>
        </header>
      ...
      {!hideUI && (
        <header className="w-full max-w-4xl flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap justify-center sm:justify-end">
            ...
            {canEditPlayers && (
              <button
                onClick={() => setCurrentPage('organiser')}
                className="px-4 py-3 sm:py-2.5 bg-slate-800 hover:bg-slate-700 text-white rounded-xl text-xs font-black uppercase tracking-tight transition-all flex items-center gap-2 min-h-[44px]"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
