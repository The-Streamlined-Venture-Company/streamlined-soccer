...
const Pitch: React.FC<PitchProps> = ({ children, isPortrait = false }) => {
  return (
    <div className="relative w-full h-full border-[8px] border-white/95 overflow-hidden shadow-[0_30px_60px_-12px_rgba(0,0,0,0.7)] rounded-sm group">
      ...
      {/* Field Markings - Landscape (default) */}
      {!isPortrait && (
      ...
      {/* Field Markings - Portrait (rotated 90° - goals at top/bottom) */}
      {isPortrait && (
        <div className="absolute inset-0 pointer-events-none opacity-85">
          {/* Center Line (horizontal in portrait) */}
          <div className="absolute top-1/2 left-0 right-0 h-2 bg-white/90 -translate-y-1/2 shadow-[0_0_10px_rgba(255,255,255,0.2)]" />

          {/* Center Circle */}
          <div className="absolute left-1/2 top-1/2 w-[35%] aspect-square border-[3.5px] border-white/90 rounded-full -translate-x-1/2 -translate-y-1/2 flex items-center justify-center">
             <div className="w-4 h-4 bg-white/90 rounded-full shadow-[0_0_15px_rgba(255,255,255,0.4)]" />
          </div>

          {/* Penalty Areas (Top - Black team goal) */}
          <div className="absolute top-0 left-1/4 right-1/4 h-[16%] border-x-[3.5px] border-b-[3.5px] border-white/90">
              {/* Penalty Arc (The D) */}
              <div className="absolute bottom-0 left-1/2 -translate-x-1/2 translate-y-1/2 h-[60%] aspect-square border-[3.5px] border-white/90 rounded-full"
                   style={{ clipPath: 'inset(0 0 50% 0)' }} />
              {/* Penalty Spot */}
              <div className="absolute bottom-[25%] left-1/2 -translate-x-1/2 w-2.5 h-2.5 bg-white/90 rounded-full shadow-[0_0_10px_rgba(255,255,255,0.4)]" />
          </div>
          {/* Goal Area (Top) */}
          <div className="absolute top-0 left-[38%] right-[38%] h-[6%] border-x-[3.5px] border-b-[3.5px] border-white/90 bg-white/5" />

          {/* Penalty Areas (Bottom - White team goal) */}
          <div className="absolute bottom-0 left-1/4 right-1/4 h-[16%] border-x-[3.5px] border-t-[3.5px] border-white/90">
              {/* Penalty Arc (The D) */}
              <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 h-[60%] aspect-square border-[3.5px] border-white/90 rounded-full"
                   style={{ clipPath: 'inset(50% 0 0 0)' }} />
              {/* Penalty Spot */}
              <div className="absolute top-[25%] left-1/2 -translate-x-1/2 w-2.5 h-2.5 bg-white/90 rounded-full shadow-[0_0_10px_rgba(255,255,255,0.4)]" />
          </div>
          {/* Goal Area (Bottom) */}
          <div className="absolute bottom-0 left-[38%] right-[38%] h-[6%] border-x-[3.5px] border-t-[3.5px] border-white/90 bg-white/5" />

          {/* Corner Arcs */}
          <div className="absolute top-0 left-0 w-[6%] aspect-square border-b-[3.5px] border-r-[3.5px] border-white/90 rounded-br-full" />
          <div className="absolute top-0 right-0 w-[6%] aspect-square border-b-[3.5px] border-l-[3.5px] border-white/90 rounded-bl-full" />
          <div className="absolute bottom-0 left-0 w-[6%] aspect-square border-t-[3.5px] border-r-[3.5px] border-white/90 rounded-tr-full" />
          <div className="absolute bottom-0 right-0 w-[6%] aspect-square border-t-[3.5px] border-l-[3.5px] border-white/90 rounded-tl-full" />
        </div>
      )}

      {/* Goal Nets (Visual only) - Landscape */}
      {!isPortrait && (
        <>
      ...
      {/* Goal Nets (Visual only) - Portrait */}
      {isPortrait && (
        <>
          <div className="absolute top-[-20px] left-[38%] right-[38%] h-[20px] border-x-[2px] border-b-[2px] border-white/40 overflow-hidden">
             <div className="w-full h-full opacity-20" style={{ backgroundImage: 'radial-gradient(circle, white 1px, transparent 1px)', backgroundSize: '4px 4px' }} />
          </div>
          <div className="absolute bottom-[-20px] left-[38%] right-[38%] h-[20px] border-x-[2px] border-t-[2px] border-white/40 overflow-hidden">
             <div className="w-full h-full opacity-20" style={{ backgroundImage: 'radial-gradient(circle, white 1px, transparent 1px)', backgroundSize: '4px 4px' }} />
          </div>
        </>
      )}
