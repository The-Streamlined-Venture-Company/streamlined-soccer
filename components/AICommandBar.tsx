...
const AICommandBar: React.FC<AICommandBarProps> = ({
    ...
    try {
      ...
      if (shouldAssignToField && (imageToProcess || looksLikePlayerList(trimmedInput))) {
        ...
        if (playerNames.length > 0 && onAssignToField && findPlayerByName) {
          ...
          setResponse(`✓ Assigned ${playerNames.length} players • Black: ${balanced.black.totalScore} pts • White: ${balanced.white.totalScore} pts`);
  ...
  return (
    <div className="w-full max-w-4xl mx-auto">
      ...
      {response && (
        <div
          className={`mb-4 p-4 rounded-2xl text-sm font-medium transition-all ${
            isError
      ...
      {/* Image preview */}
      {selectedImage && (
        <div className="mb-3 flex items-center gap-2 bg-slate-800/50 rounded-xl p-3 border border-slate-700">
      ...
      {/* Main input bar */}
      <div className="relative bg-slate-900/80 backdrop-blur-sm rounded-2xl border border-slate-700/50 shadow-xl overflow-hidden">
        <div className="flex items-end gap-2 p-3">
          ...
          <textarea
            ...
            onPaste={handlePaste}
            placeholder="Paste player names, upload a screenshot, or ask anything..."
            disabled={isLoading}
            rows={1}
        ...
        <div className="px-4 pb-3 flex items-center justify-center gap-4 text-[10px] text-slate-600 font-medium uppercase tracking-wider">
          <span className="flex items-center gap-1">
            <kbd className="px-1.5 py-0.5 bg-slate-800 rounded text-slate-500">⌘V</kbd>
            Paste list
          </span>
          <span className="flex items-center gap-1">
